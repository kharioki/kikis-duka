 ...
 * This variable will only have a value after retrieving the discount codes from the API.
 */
...
export function QRCodeForm({ QRCode: InitialQRCode }) {
  ...
  const QRCodeURL = QRCode
    ? new URL(`/qrcodes/${QRCode.id}/image`,
    location.toString()).toString()
    : null;
   ...
   * Finally, it closes the ResourcePicker.
   */
  ...
  const originalImageSrc = selectedProduct?.images?.[0]?.node?.originalSrc;
  ...
  return (
    <Stack vertical>
      {deletedProduct && (
        <Banner
          title="This product for this QR code no longer exists"
          status="critical"
        >
          <p>Scans will be directed to a 404 page, or you can choose another product for this QR code.</p>
        </Banner>
      )}
      <Layout>
        <Layout.Section>
                ...
                <Card.Section>
                  ...
                  {productId.value ? (
                    ...
                    <Stack vertical spacing="extraTight">
                      ...
                      {productId.error && (
                        <Stack spacing="tight">
                          <Icon source={AlertCircleMinor} color="critical" />
                ...
                <Card.Section title="Scan Destination">
                  <ChoiceList
                    ...
                    choices={[
                      ...
                      { label: "Link to checkout page with product in the cart", value: "checkout" },
                    ]}
...
function productViewURL({ host, productHandle, discountCode }) {
  ...
  /**
   * If a discount is selected, then build a URL to the selected discount that redirects to the selected product.
   * : /discount/{code}?redirect=/products/{product}
   */
  if (discountCode) {
...
/** Builds a URL to a checkout with the selected product */
function productCheckoutURL({ host, variantId, quantity = 1, discountCode }) {
  ...
  /** Builds a URL to a checkout that containds the selected product with a discount code applied */
  if (discountCode) {
